...
export function applyForce(angle: number, unit: Unit, initialSpeed: number, config?: ApplyForceConfig) {
    ...
    if (!forceDummyUnit) {
        forceDummyUnit = Unit.create(Players[PlayerIndex.Neutral], WTS_Units.GroundDummy, defaultX, defaultY);
    }
    ...
    timer.start(refreshInterval, true, () => {
        ...
        if (forceDummyUnit && config?.strictPathing) {
            ...
            if (Math.floor(forceDummyUnit.x) !== Math.floor(unit.x) || Math.floor(forceDummyUnit.y) !== Math.floor(unit.y)) {
                // print(`Collision detected! Dummy x: ${forceDummyUnit.x} y: ${forceDummyUnit.y} Unit x: ${unit.x} y: ${unit.y}`);
                const e = Effect.create("Abilities\\Spells\\Other\\TalkToMe\\TalkToMe", forceDummyUnit.x, forceDummyUnit.y);
                e?.setScaleMatrix(2, 2, 2);
                destroyForceEffect(true);
                return;
            }
 ...
 * @param affectHeight determines whether or not to change unit height whilst force is applied
 */
export function applyForceForEffect(angle: number, effect: Effect, initialSpeed: number, config?: ApplyForceConfig) {
    const timer = Timer.create();
    const refreshInterval = 0.01;
    const updatesPerSecond = 1 / refreshInterval;
